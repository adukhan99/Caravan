...
Caravan uses a TOML configuration file located at `~/.caravan/config.toml`. This file allows you to set default models, providers, and API keys.

## File Location

- **Linux/macOS**: `~/.caravan/config.toml`
- **Windows**: Not explicitly supported, but defaults to `./.caravan/config.toml` if `HOME` is not set.
...
| `strict_mode` | Integer | Controls tool safety and visibility (see below). | `CARAVAN_STRICT_MODE` |
...
Caravan checks environment variables before looking at the config file. For example:
- `OPENAI_API_KEY` will override `openai_api_key` in the TOML.
- `MAX_TURNS` will override `max_turns` in the TOML.
- `CARAVAN_STRICT_MODE` will override `strict_mode` in the TOML.

## Command Line Arguments

Flags passed to the `caravan` binary will always take the highest precedence.

```bash
# Use a specific model regardless of config
dune exec caravan -- --model gpt-4o
```
